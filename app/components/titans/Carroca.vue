<template>
  <div class="background-border">
    <div class="min-vw-100 background-gif-carroca">
      <div class="row w-100 ">
...
onMounted(async () => {
  const titans = await getTitanByIds([9])
  titanName.value = titans.find(titan => titan.id === 9)?.name
  altura.value = titans.find(titan => titan.id === 9)?.height
  skils.value = titans.find(titan => titan.id === 9)?.abilities
  alianca.value = titans.find(titan => titan.id === 9)?.allegiance
  const characters = await getCharacterByIds([88])
  charActual.value = characters.find(character => character.id === 88)?.name
})

useCarousel('#titanCarousel')
const titanimg = ['https://static0.cbrimages.com/wordpress/wp-content/uploads/2020/12/Cart-Titan-Header.jpg'
]
