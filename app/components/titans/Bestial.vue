<template>
  <div class="background-border">
    <div class="min-vw-100 background-gif-bestial">
      <div class="row w-100 ">
...
onMounted(async () => {
  const titans = await getTitanByIds([7])
  titanName.value = titans.find(titan => titan.id === 7)?.name
  altura.value = titans.find(titan => titan.id === 7)?.height
  skils.value = titans.find(titan => titan.id === 7)?.abilities
  alianca.value = titans.find(titan => titan.id === 7)?.allegiance
  const characters = await getCharacterByIds([184, 97])
  charActual.value = characters.find(character => character.id === 184)?.name
  char2.value = characters.find(character => character.id === 97)?.name

})
useCarousel('#titanCarousel')
const titanimg = ['https://static0.cbrimages.com/wordpress/wp-content/uploads/2021/04/Beast-Titan-Season-4-.jpg'
]
...
.background-gif-bestial {
  background-image: url("/bestial.gif");
...
<!-- <template>
  <div class="background-gif-bestial">
    <h1>bestial</h1>
  </div>
</template>
